...
select 'GWTB_MSG_IN_HISTORY_Purge_'||to_char(sysdate,'ddmmyyyy')||'.SPL' spoolname from dual;
...
Declare
  ...
  Cursor CR_PURGE_DATA IS
    select b.rowid--, b.*
      from fcc114.GWTB_MSG_IN_HISTORY b
     where trunc(b.server_dt_stamp) <= trunc(sysdate-365);
...
BEGIN
  ...
  Loop
    ...
    IF l_tb_to_be_deleted.COUNT > 0 THEN
     ...
     FORALL k IN l_tb_to_be_deleted.FIRST .. l_tb_to_be_deleted.LAST   SAVE EXCEPTIONS
	  insert into fcc114.GWTB_MSG_IN_HIST_arch SELECT *
              FROM fcc114.GWTB_MSG_IN_HISTORY 
             where ROWID = l_tb_to_be_deleted(k).ROWID;

    --Deletion from the original table
	FORALL k IN l_tb_to_be_deleted.FIRST .. l_tb_to_be_deleted.LAST   SAVE EXCEPTIONS
	  DELETE FROM fcc114.GWTB_MSG_IN_HISTORY WHERE ROWID = l_tb_to_be_deleted(k).ROWID;
  ...
  END LOOP;
  COMMIT;
  dbms_output.put_line('The total no of Record deleted from GWTB_MSG_IN_HISTORY is '||cnt);
  CLOSE CR_PURGE_DATA;
