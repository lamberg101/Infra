...
select 'FBTB_TXNLOG_MASTER_HIST_Purge_'||to_char(sysdate,'ddmmyyyy')||'.SPL' spoolname from dual;
...
Declare
  ...
  Cursor CR_PURGE_DATA IS
    select b.rowid--, b.*
      from fcc114.FBTB_TXNLOG_MASTER_HIST b
     where b.postingdate <= trunc(sysdate-365);
...
BEGIN
  ...
  Loop
    ...
    IF l_tb_to_be_deleted.COUNT > 0 THEN
     ...
     FORALL k IN l_tb_to_be_deleted.FIRST .. l_tb_to_be_deleted.LAST   SAVE EXCEPTIONS
	  insert into fcc114.FBTB_TXNLOG_MASTER_HIST_arch SELECT *
              FROM fcc114.FBTB_TXNLOG_MASTER_HIST 
             where ROWID = l_tb_to_be_deleted(k).ROWID;

    --Deletion from the original table
	FORALL k IN l_tb_to_be_deleted.FIRST .. l_tb_to_be_deleted.LAST   SAVE EXCEPTIONS
	  DELETE FROM fcc114.FBTB_TXNLOG_MASTER_HIST WHERE ROWID = l_tb_to_be_deleted(k).ROWID;
  ...
  END LOOP;
  COMMIT;
  dbms_output.put_line('The total no of Record deleted from FBTB_TXNLOG_MASTER_HIST is '||cnt);
  CLOSE CR_PURGE_DATA;
