...
select 'SWTB_TXN_HIST_PURGE_'||to_char(sysdate,'ddmmyyyy')||'.SPL' spoolname from dual;
...
Declare
  ...
  Cursor CR_PURGE_DATA IS
    select b.rowid, b.*
      from fcc114.SWTB_TXN_HIST b
     where b.purge_date <= trunc(sysdate-365);
...
BEGIN
  ...
  Loop
	...
	FORALL k IN l_tb_to_be_deleted.FIRST .. l_tb_to_be_deleted.LAST   SAVE EXCEPTIONS
	  DELETE FROM fcc114.SWTB_TXN_HIST WHERE ROWID = l_tb_to_be_deleted(k).ROWID;
  ...
  END LOOP;
  COMMIT;
  dbms_output.put_line('The total no of Record deleted from SWTB_TXN_HIST is '||cnt);
  CLOSE CR_PURGE_DATA;
